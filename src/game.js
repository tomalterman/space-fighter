...
const ROCKET_BULLET_SPEED   = 4.5;
const ROCKET_BLAST_RADIUS   = 60;
...
const ENEMY_STATS = {
    ...
    cyborg:  { w: 48, h: 56, hp: 3,  score: 5000, fireCdMin: 70,  fireCdMax: 110, dropChance: 1.00 }
};
...
const WAVE_SCHEDULE = [
    ...
    { at: 2340, kind: 'bossWave' },
    { at: 2520, kind: 'turretOnTerrain', offsets: [40, 130, 254, 344] }
];

// Boss encounter progression — advances each time a bossWave spawns. The
// final entry repeats; existing difficultyLoop scaling keeps it getting
// harder.
const BOSS_PROGRESSION = [
    // 1st boss: lone bomber (the original "boss" from v16 and earlier).
    [ { type: 'bomber', x: 'center',    y: -36,  offsetX:  0  } ],
    // 2nd boss: the new cyborg makes its debut.
    [ { type: 'cyborg', x: 'center',    y: -50,  offsetX:  0  } ],
    // 3rd: cyborg + bomber escorts in side by side.
    [
        { type: 'cyborg', x: 'center', y:  -50, offsetX: -60 },
        { type: 'bomber', x: 'center', y:  -40, offsetX:  50 }
    ],
    // 4th: double cyborg, staggered.
    [
        { type: 'cyborg', x: 'center', y:  -50, offsetX: -70 },
        { type: 'cyborg', x: 'center', y:  -90, offsetX:  70 }
    ],
    // 5th: 2 cyborgs + a bomber.
    [
        { type: 'cyborg', x: 'center', y:  -50, offsetX: -80 },
        { type: 'cyborg', x: 'center', y:  -90, offsetX:  80 },
        { type: 'bomber', x: 'center', y: -130, offsetX:   0 }
    ],
    // 6th+: 2 cyborgs + 2 bombers (then repeats under scaled difficulty).
    [
        { type: 'cyborg', x: 'center', y:  -50, offsetX: -80 },
        { type: 'cyborg', x: 'center', y:  -90, offsetX:  80 },
        { type: 'bomber', x: 'center', y: -140, offsetX: -40 },
        { type: 'bomber', x: 'center', y: -140, offsetX:  40 }
    ]
];
...
function gameInit() {
    ...
    // Replace the engine's four directional touch buttons with a single
    // virtual d-pad widget, half-size fire/bomb buttons, and a layout
    // pulled in toward the center for young kids holding an iPad.
    setupTouchControls();
...
function updatePlayer(dt) {
    ...
    if (Engine.input.fire && player.fireCooldown <= 0) {
        ...
        if (player.weapon === 'laser') {
            ...
            player.fireCooldown = ROCKET_FIRE_COOLDOWN;
        } else {
...
function firePlayerWeapon() {
    ...
    if (player.weapon === 'vulcan') {
        ...
        // Rocket: slow fire rate, big AOE. Tracer vulcans fill in some
        // gap-fire at higher power so the player is never defenseless.
        const power = player.power;
        if (power === 1) {
            spawnRocket(px, py, 0, -ROCKET_BULLET_SPEED);
        } else if (power === 2) {
            spawnRocket(px, py, 0, -ROCKET_BULLET_SPEED);
            spawnVulcan(px - 6, py + 2, -0.4, -VULCAN_BULLET_SPEED);
            spawnVulcan(px + 6, py + 2,  0.4, -VULCAN_BULLET_SPEED);
        } else if (power === 3) {
            spawnRocket(px - 5, py + 2, -0.35, -ROCKET_BULLET_SPEED);
            spawnRocket(px + 5, py + 2,  0.35, -ROCKET_BULLET_SPEED);
        } else if (power === 4) {
            spawnRocket(px - 5, py + 2, -0.35, -ROCKET_BULLET_SPEED);
            spawnRocket(px + 5, py + 2,  0.35, -ROCKET_BULLET_SPEED);
            spawnVulcan(px - 9, py + 4, -0.8, -VULCAN_BULLET_SPEED);
            spawnVulcan(px + 9, py + 4,  0.8, -VULCAN_BULLET_SPEED);
        } else {
            spawnRocket(px,     py,    0,     -ROCKET_BULLET_SPEED);
            spawnRocket(px - 7, py + 3, -0.45, -ROCKET_BULLET_SPEED * 0.95);
            spawnRocket(px + 7, py + 3,  0.45, -ROCKET_BULLET_SPEED * 0.95);
        }
        Engine.Sound.play('shoot');
        // Extra rocket-launch smoke
        Engine.spawnParticle(px, py + 1, 0, 1.5, 2, COLOR.ROCKET_TRAIL, 12);
        Engine.spawnParticle(px - 1, py + 2, -0.6, 1.2, 1, COLOR.ROCKET_TRAIL, 10);
        Engine.spawnParticle(px + 1, py + 2, 0.6, 1.2, 1, COLOR.ROCKET_TRAIL, 10);
    }
        ...
        if (b.kind === 'rocket') {
            ...
            if (b.trailTick >= 2) {
                ...
                Engine.spawnParticle(
                    ...
                    0.4 + Math.random() * 0.3,
                    1 + Math.random(),
                    Math.random() < 0.5 ? COLOR.ROCKET_TRAIL : '#e08a3a',
                    10 + Math.random() * 6
                );
            }
            if (b.y < -20) {
                // Detonate on ceiling exit so the player gets a visible
                // burst even on overshoots. Splash-damages anything on
                // screen near the top.
                explodeRocket(b.x, 4, b.damage);
...
function renderPlayerBullets(ctx) {
    for (const b of playerBullets) {
        if (b.kind === 'vulcan') {
            ...
            // Rocket (player projectile): pointed-up missile with flame.
            const flick = (Math.floor(b.age * 0.5) % 2) === 0;
            // Flame tail
            ctx.fillStyle = flick ? '#ffea3a' : '#ff7a2a';
            ctx.fillRect(b.x - 1, b.y + 4, 2, 3);
            ctx.fillStyle = '#fff3a0';
            ctx.fillRect(b.x - 0.5, b.y + 4, 1, flick ? 3 : 2);
            // Body
            ctx.fillStyle = COLOR.CYBORG_DARK;
            ctx.fillRect(b.x - 2, b.y - 4, 4, 8);
            ctx.fillStyle = COLOR.ROCKET_BODY;
            ctx.fillRect(b.x - 1.5, b.y - 4, 3, 8);
            ctx.fillStyle = COLOR.ROCKET_HI;
            ctx.fillRect(b.x - 1.5, b.y - 4, 1, 8);
            // Nose
            ctx.fillStyle = COLOR.CYBORG_EYE;
            ctx.fillRect(b.x - 1, b.y - 5, 2, 1);
            // Fins
            ctx.fillStyle = COLOR.CYBORG_DARK;
            ctx.fillRect(b.x - 3, b.y + 2, 1, 2);
            ctx.fillRect(b.x + 2, b.y + 2, 1, 2);
        }
    }
}
...
function makeEnemy(type, x, y) {
    ...
    return {
        ...
        amp: 0,
        phase: 0,
        // Cyborg bookkeeping (ignored by other enemy types).
        fireTelegraph: 0,
        lastFireSide: 0,
        hitFlash: 0
    };
...
function updateEnemies(dt) {
    for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        if (e.pattern === 'sine') {
            ...
            e.y += SCROLL_SPEED * dt;
        } else if (e.pattern === 'cyborgHover') {
            // Descend to a hover height, then drift left/right. After a
            // long timeout, accelerate downward so stuck cyborgs exit the
            // screen even if the player ignores them.
            const targetY = 60;
            if (e.y < targetY) {
                e.y += e.vy * dt;
            } else {
                e.x = clamp(
                    e.anchorX + Math.sin(e.age * 0.025) * 90,
                    30, GAME.width - 30
                );
                if (e.age > 1400) {
                    e.y += (e.vy + (e.age - 1400) * 0.002) * dt;
                }
            }
        } else {
            e.x += e.vx * dt;
        ...
        if (e.fireTelegraph > 0) e.fireTelegraph -= dt;
        if (e.hitFlash > 0) e.hitFlash -= dt;
        const onScreen = e.y > 4 && e.y < GAME.height - 30;
        // Cyborgs briefly telegraph before firing — arms lift and the
        // reactor glows so a 6-year-old can see the rocket coming.
        if (e.type === 'cyborg' && e.fireCooldown > 0 && e.fireCooldown < 18 && e.fireTelegraph <= 0 && onScreen) {
            e.fireTelegraph = 18;
        }
...
function enemyFire(e) {
    ...
    if (e.type === 'popcorn') {
        ...
        // Alternate arms so the visual lines up with the rocket origin.
        const side = (e.lastFireSide === 1) ? -1 : 1;
        e.lastFireSide = side;
        // Muzzle is at the tip of the lifted launcher arm.
        const muzzleX = e.x + side * 18;
        const muzzleY = e.y + 6;
        spawnEnemyRocket(muzzleX, muzzleY, speed * 1.2);
        // Muzzle spark
        Engine.spawnParticle(muzzleX, muzzleY, side * 0.6, 0.3, 2, COLOR.ROCKET_HI, 8);
        e.fireTelegraph = 0;
        Engine.Sound.play('enemyShoot');
    }
}
...
function renderEnemies(ctx) {
    for (const e of enemies) {
        ...
        else if (e.type === 'cyborg') drawCyborg(ctx, e);
    }
}
...
function drawCyborg(ctx, e) {
    const x = e.x, y = e.y;
    const age = e.age;
    const telegraph = e.fireTelegraph > 0;
    // Hit flash (recent damage) brightens the chassis for a few frames.
    const hurt = e.hitFlash > 0;

    // ---- Skirt / base with thruster flicker --------------------------
    const flick = (Math.floor(age * 0.5) % 2) === 0;
    ctx.fillStyle = COLOR.CYBORG_DARK;
    ctx.fillRect(x - 14, y + 18, 28, 8);
    ctx.fillStyle = COLOR.CYBORG_BODY;
    ctx.fillRect(x - 13, y + 18, 26, 6);
    // Thrusters underneath
    ctx.fillStyle = flick ? COLOR.CYBORG_NEON : COLOR.JET_CORE;
    ctx.fillRect(x - 9, y + 24, 3, 2);
    ctx.fillRect(x + 6, y + 24, 3, 2);
    ctx.fillStyle = '#ff9a3a';
    ctx.fillRect(x - 9, y + 26, 3, flick ? 2 : 1);
    ctx.fillRect(x + 6, y + 26, 3, flick ? 1 : 2);

    // ---- Central torso ----------------------------------------------
    ctx.fillStyle = COLOR.CYBORG_DARK;
    ctx.fillRect(x - 11, y - 4, 22, 24);
    ctx.fillStyle = hurt ? COLOR.CYBORG_HI : COLOR.CYBORG_BODY;
    ctx.fillRect(x - 10, y - 3, 20, 22);
    // Chest panels (mechanical detail)
    ctx.fillStyle = COLOR.CYBORG_HI;
    ctx.fillRect(x - 8, y - 1, 7, 7);
    ctx.fillRect(x + 1, y - 1, 7, 7);
    ctx.fillStyle = COLOR.CYBORG_JOINT;
    ctx.fillRect(x - 1, y - 4, 2, 24);
    // Neon ribcage line
    ctx.fillStyle = telegraph ? '#ff6b76' : COLOR.CYBORG_NEON;
    ctx.fillRect(x - 9, y + 8, 18, 2);
    // Reactor core (glows red-orange when about to fire)
    ctx.fillStyle = telegraph ? '#ff3a3a' : '#ff8a3a';
    ...
    // Visor band with glowing eye
    ctx.fillStyle = COLOR.CYBORG_JOINT;
    ctx.fillRect(x - 5, y - 12, 10, 4);
    ctx.fillStyle = telegraph ? COLOR.CYBORG_EYE_HI : COLOR.CYBORG_EYE;
    ctx.fillRect(x - 4, y - 11, 8, 2);
    // Eye highlight (scans across)
    const scan = Math.floor((age * 0.12) % 7) - 3;
    ctx.fillStyle = COLOR.CYBORG_EYE_HI;
    ctx.fillRect(x + scan, y - 11, 2, 2);
    // Antennae
    ctx.fillStyle = COLOR.CYBORG_JOINT;
    ctx.fillRect(x - 5, y - 19, 2, 3);
    ctx.fillRect(x + 3, y - 19, 2, 3);
    ctx.fillStyle = COLOR.CYBORG_NEON;
    ctx.fillRect(x - 5, y - 19, 2, 1);
    ctx.fillRect(x + 3, y - 19, 2, 1);

    // ---- Arms (rotated groups) --------------------------------------
    // Idle sway; when telegraphing, lift outward so the rocket tube
    // points forward. e.lastFireSide tracks which arm fires next so we
    // alternate for readability.
    const sway = Math.sin(age * 0.05) * 0.18;
    const leftAim  = (telegraph && e.lastFireSide !== -1) ? -0.55 : 0;
    const rightAim = (telegraph && e.lastFireSide !==  1) ?  0.55 : 0;

    drawCyborgArm(ctx, x - 11, y - 2, -1, sway + leftAim);
    drawCyborgArm(ctx, x + 11, y - 2,  1, -sway + rightAim);
}

// Draw one mechanical arm. dir is -1 for left, +1 for right. `extraAngle`
// is a small rotation applied to the whole arm for sway / firing telegraph.
function drawCyborgArm(ctx, sx, sy, dir, extraAngle) {
    ctx.save();
    ctx.translate(sx, sy);
    // Shoulder pivots so arm dangles downward by default, outward when
    // aiming. Rotate around the shoulder: 0 = straight down.
    ctx.rotate(extraAngle * dir);
    ...
    // Forearm — rocket launcher tube (kinks outward slightly)
    ctx.fillStyle = COLOR.CYBORG_DARK;
    ctx.fillRect(-4, 14, 8, 10);
    ctx.fillStyle = COLOR.CYBORG_BODY;
    ctx.fillRect(-3, 14, 6, 9);
    ctx.fillStyle = COLOR.CYBORG_HI;
    ctx.fillRect(-3, 14, 1, 9);
    // Twin rocket tubes at the tip
    ctx.fillStyle = COLOR.CYBORG_JOINT;
    ctx.fillRect(-3, 22, 2, 4);
    ctx.fillRect( 1, 22, 2, 4);
    // Red warning light on launcher
    ctx.fillStyle = COLOR.CYBORG_EYE;
...
function spawnWave(wave) {
    ...
    switch (wave.kind) {
        ...
        case 'bossWave': {
            // Look up the current boss composition and spawn each member.
            // The final table entry repeats; existing difficultyLoop
            // scaling keeps making each encounter harder beyond that.
            const pattern = BOSS_PROGRESSION[
                Math.min(bossEncounterIndex, BOSS_PROGRESSION.length - 1)
            ];
            bossEncounterIndex++;
            const cx = GAME.width / 2;
            for (const spec of pattern) {
                const baseX = (spec.x === 'center') ? cx : spec.x;
                const e = makeEnemy(spec.type, baseX + (spec.offsetX || 0), spec.y);
                e.anchorX = baseX + (spec.offsetX || 0);
                if (spec.type === 'cyborg') {
                    e.pattern = 'cyborgHover';
                    e.vx = 0;
                    e.vy = 0.55 * speedMul;
                } else {
                    e.pattern = 'down';
...
// Cyborg rocket — aimed at the player at spawn time, leaves a smoke
// trail, same circular player-collision as regular enemy bullets so no
// extra collision code is needed.
function spawnEnemyRocket(x, y, speed) {
    if (!player) return;
    const dx = player.hitboxX - x;
    const dy = player.hitboxY - y;
    const len = Math.sqrt(dx * dx + dy * dy) || 1;
    enemyBullets.push({
        ...
        vy: (dy / len) * speed,
        r: 4,
        color: COLOR.ROCKET_BODY,
        glowColor: COLOR.ROCKET_HI,
        age: 0,
        trailTick: 0
    });
}
...
function drawEnemyRocket(ctx, b) {
    const angle = Math.atan2(b.vy, b.vx);
    ctx.save();
    ctx.translate(b.x, b.y);
    // Rocket points in the direction of travel (+x of our local frame).
    ctx.rotate(angle);
    // Back flame (flickers)
    const flick = (Math.floor(b.age * 0.5) % 2) === 0;
    ctx.fillStyle = flick ? '#ffea3a' : '#ff7a2a';
...
function collidePlayerBulletsVsEnemies() {
    for (let i = playerBullets.length - 1; i >= 0; i--) {
        ...
        if (b.kind === 'rocket') {
            // Rockets detonate on first enemy overlap with AOE.
            let hit = false;
            ...
            if (hit) {
                explodeRocket(b.x, b.y, b.damage);
                playerBullets.splice(i, 1);
            }
            continue;
        }
...
// AOE detonation: big visual, screen shake, and damage to every enemy
// within ROCKET_BLAST_RADIUS of (x, y).
function explodeRocket(x, y, damage) {
    spawnExplosion(x, y, true);
    spawnExplosion(x - 14, y + 8, false);
    spawnExplosion(x + 14, y - 6, false);
    Engine.Sound.play('bigExplode');
    shakeFrames = Math.max(shakeFrames, SHAKE_FRAMES * 0.6);
    // Ring particles so the AOE edge is visible for a frame or two.
    for (let i = 0; i < 12; i++) {
        ...
        Engine.spawnParticle(
            ...
            Math.sin(a) * 2.2,
            1 + Math.random(),
            Math.random() < 0.5 ? COLOR.EXPLODE_YELLOW : COLOR.ROCKET_BODY,
            14 + Math.random() * 8
        );
    }
    const R2 = ROCKET_BLAST_RADIUS * ROCKET_BLAST_RADIUS;
    for (let k = enemies.length - 1; k >= 0; k--) {
        const e = enemies[k];
        const dx = e.x - x, dy = e.y - y;
        if (dx * dx + dy * dy <= R2) {
            e.hp -= damage;
            if (e.type === 'cyborg') e.hitFlash = 6;
...
function killEnemy(index) {
    ...
    if (e.type === 'bomber') {
        ...
        // Triple explosion for the "big kill" feel + screen shake.
        spawnExplosion(e.x, e.y, true);
        spawnExplosion(e.x - 12, e.y + 6, false);
        spawnExplosion(e.x + 12, e.y - 4, false);
        Engine.Sound.play('bigExplode');
        shakeFrames = SHAKE_FRAMES;
        // Cyborg always drops the rocket weapon powerup.
        spawnPowerup(e.x, e.y, 'R');
        return;
    } else {
...
function spawnPowerup(x, y, forcedKind) {
    const kind = forcedKind != null
        ? forcedKind
...
function powerupColor(kind) {
    ...
    if (kind === 'R')    return COLOR.POWER_ROCKET;
    return '#ffffff';
}
...
// =====================================================================
// TOUCH CONTROLS — virtual d-pad + half-size fire/bomb
// =====================================================================
//
...
// row. Six buttons in a row is unusable for a young kid holding an
// iPad. Replace it with:
//
//   1. A single circular VIRTUAL D-PAD on the bottom left. Touching
//      anywhere inside it sets Engine.input.up/down/left/right based on
//      the touch position relative to the dpad center, with a small
//      dead zone in the middle. Touchmove updates direction live so a
//      thumb can slide between directions without lifting. Multi-touch
//      safe via touch.identifier so a finger on the d-pad doesn't
//      break a separate finger on FIRE.
//   2. The engine-generated #touch_up/down/left/right divs are hidden
//      with display:none so they don't double-fire.
//   3. FIRE and BOMB are half their previous size and pulled in
//      toward the center so a kid's thumbs reach all the controls
//      without stretching to the corners.
...
function setupTouchControls() {
    ...
    // 1. Inject the layout stylesheet.
    ...
    style.textContent = `
        @media (pointer: coarse) {
            ...
            /* The engine still generates these for keyboard parity,
            ...
            /* --- Phone defaults --- */
            #virtualDpad { width: 88px; height: 88px; bottom: 18px; left: 18px; }
            #touch_fire {
        ...
        /* --- Tablet (>=700px): smaller corner-hugging d-pad on the
               bottom-left, FIRE/BOMB on the bottom-right. --- */
        @media (pointer: coarse) and (min-width: 700px) {
            #virtualDpad { width: 120px; height: 120px; bottom: 30px; left: 30px; }
            #touch_fire {
    ...
    // 2. Build and append the d-pad widget.
    const dpad = document.createElement('div');
    dpad.id = 'virtualDpad';
    touchControls.appendChild(dpad);

    // 3. Touch handling — multi-touch safe via touch.identifier.
    let dpadTouchId = null;

    function setDpadFromTouch(touch) {
        const rect = dpad.getBoundingClientRect();
        const cx = rect.left + rect.width / 2;
        const cy = rect.top + rect.height / 2;
        const dx = touch.clientX - cx;
        const dy = touch.clientY - cy;
        // Per-axis dead zone — naturally creates a square dead area in
        // the center and supports 8-direction (touch in upper-right of
        // d-pad sets both up and right). Small dead zone so a kid's
        // thumb activates a direction quickly.
        const deadZone = rect.width * 0.13;
        Engine.input.left  = dx < -deadZone;
        Engine.input.right = dx >  deadZone;
        Engine.input.up    = dy < -deadZone;
        Engine.input.down  = dy >  deadZone;
    }
    ...
    dpad.addEventListener('touchstart', (e) => {
        e.preventDefault();
        // Mirror the engine's title/game-over handling so the d-pad can
        // also start a game when tapped from the title screen.
